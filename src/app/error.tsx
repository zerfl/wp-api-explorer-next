...
export default function Error({
  ...
  return (
    <div
      role="alert"
      className="flex-grow flex items-center justify-center p-6 bg-background text-foreground"
    >
      <div className="max-w-md w-full border border-border/60 bg-card/30 backdrop-blur-md rounded-xl p-6 shadow-xl text-center space-y-5">
        ...
        {error.message && (
          <pre className="text-xs font-mono bg-destructive/5 text-destructive/90 p-3 rounded-lg border border-destructive/15 overflow-x-auto text-left leading-relaxed">
            {error.message}
          </pre>
        )}
